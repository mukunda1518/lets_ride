...
from lets_ride.interactors.share_ride_interactor \
    import ShareRideInteractor
...
def test_share_ride():
    ...
    interactor = ShareRideInteractor(storage=storage)

    # Assert
    interactor.share_ride_wrapper(
        user_id=user_id,
        source=source,
    ...
    # Act
    storage.create_share_ride.assert_called_once_with(
        user_id=user_id,
        source=source,
        destination=destination,
...
def test_share_ride_with_flexible_timings():
    # Arrange
    user_id =  1
    source = "kurnool"
    destination = "Hyderabad"
    ...
    # Act
    storage.create_share_ride_with_flexible_timings.assert_called_once_with(
        user_id=user_id,
        source=source,
        destination=destination,
        flexible_timings=flexible_timings,
